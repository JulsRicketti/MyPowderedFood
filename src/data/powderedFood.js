export const powderedFood = [
  {
    // Note to self, this is the vegan steady shakes
    // they all seem to have the same value
    id: 1,
    brand: 'Queal',
    product: 'Steady Shake (Vegan)',
    site: 'https://queal.com/',
    calories: 424,
    priceAndServings: {
      fullPrice: {
        EUR: 8.50
      },
      primaryCurrency: 'EUR',
      servings: 5
    },
    accomodatedRestrictions: [
      'vegan', 'lactoseIntolerance', 'nutFree', 'glutenFree', 'kosher', 'halal'
    ],
    macronutrients: {
      fat: {
        total: 15.5,
        saturated: 1.7
      },
      carbohydrates: {
        total: 49.2,
        sugars: 13.9
      },
      protein: 18.7,
      fibre: 7.2,
      sodium: 270
    },
    vitaminsAndMinerals: {
      vitaminA: {
        name: 'Vitamin A',
        quantity: 152,
        unit: 'μg',
        dailyPercentage: 20
      },
      vitaminD: {
        name: 'Vitamin D',
        quantity: 3.03,
        unit: 'μg',
        dailyPercentage: 20
      },
      vitaminE: {
        name: 'Vitamin E',
        quantity: 2.63,
        unit: 'μg',
        dailyPercentage: 20
      },
      vitaminK: {
        name: 'Vitamin K',
        quantity: 1.41,
        unit: 'μg',
        dailyPercentage: 20
      },
      vitaminC: {
        name: 'Vitamin C',
        quantity: 22.2,
        unit: 'mg',
        dailyPercentage: 20
      },
      thiamine: {
        name: 'Thiamine',
        quantity: 0.27,
        unit: 'mg',
        dailyPercentage: 23
      },
      riboflavine: {
        name: 'Riboflavine',
        quantity: 0.32,
        unit: 'mg',
        dailyPercentage: 20
      },
      niacine: {
        name: 'Niacine',
        quantity: 3.23,
        unit: 'mg',
        dailyPercentage: 20
      },
      vitaminB6: {
        name: 'Vitamin B6',
        quantity: 0.34,
        unit: 'mg',
        dailyPercentage: 20
      },
      folicAcid: {
        name: 'Folic Acid',
        quantity: 51,
        unit: 'μg',
        dailyPercentage: 20
      },
      vitaminB12: {
        name: 'Vitamin B12',
        quantity: 0.81,
        unit: 'μg',
        dailyPercentage: 33
      },
      biotine: {
        name: 'Biotine',
        quantity: 8.08,
        unit: 'μg',
        dailyPercentage: 20
      },
      vitaminB5: {
        name: 'Vitamin B5',
        quantity: '?',
        unit: 'mg',
        dailyPercentage: 20
      },
      potassium: {
        name: 'Potassium',
        quantity: 710,
        unit: 'mg',
        dailyPercentage: 20
      },
      chloride: {
        name: 'Chloride',
        quantity: 460,
        unit: 'mg',
        dailyPercentage: 20
      },
      calcium: {
        name: 'Calcium',
        quantity: 190,
        unit: 'mg',
        dailyPercentage: 26
      },
      phosphorus: {
        name: 'Phosphorus',
        quantity: 199,
        unit: 'mg',
        dailyPercentage: 36
      },
      magnesium: {
        name: 'Magnesium',
        quantity: 71,
        unit: 'mg',
        dailyPercentage: 20
      },
      iron: {
        name: 'Iron',
        quantity: 2.22,
        unit: 'mg',
        dailyPercentage: 20
      },
      zinc: {
        name: 'Zinc',
        quantity: 3.29,
        unit: 'mg',
        dailyPercentage: 20
      },
      copper: {
        name: 'Copper',
        quantity: 0.3,
        unit: 'mg',
        dailyPercentage: 20
      },
      manganese: {
        name: 'Manganese',
        quantity: 1.6,
        unit: 'mg',
        dailyPercentage: 54
      },
      selenium: {
        name: 'Selenium',
        quantity: 14.1,
        unit: 'μg',
        dailyPercentage: 20
      },
      molybdenum: {
        name: 'Molybdenum',
        quantity: 13.1,
        unit: 'μg',
        dailyPercentage: 20
      },
      iodine: {
        name: 'Iodine',
        quantity: 30.3,
        unit: 'μg',
        dailyPercentage: 20
      },
      choline: {
        name: 'Choline',
        quantity: 81,
        unit: 'mg',
        dailyPercentage: 20
      },
      chromium: {
        name: 'Chromium',
        quantity: 0,
        unit: 'mg',
        dailyPercentage: 0
      }
    }
  },
  {
    // Note to self: this is just the chocolate hol food
    id: 2,
    brand: 'Hol Food',
    product: 'Chocolate/Vanilla',
    site: 'https://holfood.com/',
    calories: 400,
    priceAndServings: {
      fullPrice: {
        CAD: 93.34
      },
      primaryCurrency: 'CAD',
      servings: 30
    },
    accomodatedRestrictions: [
      'nutFree', 'glutenFree'
    ],
    macronutrients: {
      fat: {
        total: 9,
        saturated: 5,
        trans: 0
      },
      carbohydrates: {
        total: 44,
        sugars: 9
      },
      protein: 35,
      fibre: 5,
      sodium: 373
    },
    vitaminsAndMinerals: {
      vitaminA: {
        name: 'Vitamin A',
        quantity: 0.2204,
        unit: 'mg',
        dailyPercentage: 29
      },
      vitaminD: {
        name: 'Vitamin D',
        quantity: 3.03,
        unit: 'μg',
        dailyPercentage: 20
      },
      vitaminE: {
        name: 'Vitamin E',
        quantity: 2.63,
        unit: 'μg',
        dailyPercentage: 20
      },
      vitaminK: {
        name: 'Vitamin K',
        quantity: 0,
        unit: 'μg',
        dailyPercentage: 0
      },
      vitaminC: {
        name: 'Vitamin C',
        quantity: 22.2,
        unit: 'mg',
        dailyPercentage: 20
      },
      thiamine: {
        name: 'Thiamine',
        quantity: 0.293,
        unit: 'mg',
        dailyPercentage: 25
      },
      riboflavine: {
        name: 'Riboflavine',
        quantity: 0.432,
        unit: 'mg',
        dailyPercentage: 27
      },
      niacine: {
        name: 'Niacine',
        quantity: 4.522,
        unit: 'mg',
        dailyPercentage: 28
      },
      vitaminB6: {
        name: 'Vitamin B6',
        quantity: 0.408,
        unit: 'mg',
        dailyPercentage: 24
      },
      folicAcid: {
        name: 'Folic Acid',
        quantity: 51,
        unit: 'μg',
        dailyPercentage: 20
      },
      vitaminB12: {
        name: 'Vitamin B12',
        quantity: 0.81,
        unit: 'μg',
        dailyPercentage: 33
      },
      biotine: {
        name: 'Biotine',
        quantity: 0.025,
        unit: 'mg',
        dailyPercentage: 62.5
      },
      vitaminB5: { // Also called Pantothetic Acid
        name: 'Vitamin B5',
        quantity: '?',
        unit: 'mg',
        dailyPercentage: 25
      },
      potassium: {
        name: 'Potassium',
        quantity: 940,
        unit: 'mg',
        dailyPercentage: 26.47
      },
      chloride: {
        name: 'Chloride',
        quantity: 460,
        unit: 'mg',
        dailyPercentage: 20
      },
      calcium: {
        name: 'Calcium',
        quantity: 160,
        unit: 'mg',
        dailyPercentage: 22
      },
      phosphorus: {
        name: 'Phosphorus',
        quantity: 121.61,
        unit: 'mg',
        dailyPercentage: 22
      },
      magnesium: {
        name: 'Magnesium',
        quantity: 71,
        unit: 'mg',
        dailyPercentage: 20
      },
      iron: {
        name: 'Iron',
        quantity: 2.997,
        unit: 'mg',
        dailyPercentage: 27
      },
      zinc: {
        name: 'Zinc',
        quantity: 4.44,
        unit: 'mg',
        dailyPercentage: 27
      },
      copper: {
        name: 'Copper',
        quantity: 0.5,
        unit: 'mg',
        dailyPercentage: 33.3
      },
      manganese: {
        name: 'Manganese',
        quantity: 1,
        unit: 'mg',
        dailyPercentage: 33
      },
      selenium: {
        name: 'Selenium',
        quantity: 0.011,
        unit: 'mg',
        dailyPercentage: 15
      },
      molybdenum: {
        name: 'Molybdenum',
        quantity: 0.02,
        unit: 'mg',
        dailyPercentage: 30
      },
      iodine: {
        name: 'Iodine',
        quantity: 40.905,
        unit: 'μg',
        dailyPercentage: 27
      },
      choline: {
        name: 'Choline',
        quantity: 81,
        unit: 'mg',
        dailyPercentage: 20
      },
      chromium: {
        name: 'Chromium',
        quantity: 0.01,
        unit: 'mg',
        dailyPercentage: '?'
      }
    }
  },
  {
    id: 3,
    brand: 'Soylent',
    product: 'Powder',
    site: 'https://www.soylent.com/',
    calories: 400,
    priceAndServings: {
      fullPrice: {
        CAD: 88,
        USD: 64
      },
      primaryCurrency: 'USD',
      servings: 35
    },
    accomodatedRestrictions: [
      'vegan', 'lactoseIntolerance', 'nutFree', 'glutenFree', 'kosher', 'halal'
    ],
    macronutrients: {
      fat: {
        total: 19,
        saturated: 1.5,
        trans: 0.2,
        polysaturated: 3.5
      },
      carbohydrates: {
        total: 42,
        sugars: 16
      },
      protein: 20,
      fibre: 4,
      sodium: 320
    },
    vitaminsAndMinerals: {
      vitaminA: {
        name: 'Vitamin A',
        quantity: 0.1672,
        unit: 'mg',
        dailyPercentage: 22
      },
      vitaminD: {
        name: 'Vitamin D',
        quantity: 3.03,
        unit: 'μg',
        dailyPercentage: 20
      },
      vitaminE: {
        name: 'Vitamin E',
        quantity: 2.63,
        unit: 'μg',
        dailyPercentage: 20
      },
      vitaminK: {
        name: 'Vitamin K',
        quantity: 0,
        unit: 'μg',
        dailyPercentage: 0
      },
      vitaminC: {
        name: 'Vitamin C',
        quantity: 22.2,
        unit: 'mg',
        dailyPercentage: 20
      },
      thiamine: {
        name: 'Thiamine',
        quantity: 0.246,
        unit: 'mg',
        dailyPercentage: 21
      },
      riboflavine: {
        name: 'Riboflavine',
        quantity: 0.432,
        unit: 'mg',
        dailyPercentage: 27
      },
      niacine: {
        name: 'Niacine',
        quantity: 3.06,
        unit: 'mg',
        dailyPercentage: 19
      },
      vitaminB6: {
        name: 'Vitamin B6',
        quantity: 0.408,
        unit: 'mg',
        dailyPercentage: 24
      },
      folicAcid: {
        name: 'Folic Acid',
        quantity: 51,
        unit: 'μg',
        dailyPercentage: 20
      },
      vitaminB12: {
        name: 'Vitamin B12',
        quantity: 0.81,
        unit: 'μg',
        dailyPercentage: 33
      },
      biotine: {
        name: 'Biotine',
        quantity: 0.06,
        unit: 'mg',
        dailyPercentage: 150
      },
      vitaminB5: { // Also called Pantothetic Acid
        name: 'Vitamin B5',
        quantity: '?',
        unit: 'mg',
        dailyPercentage: 25
      },
      potassium: {
        name: 'Potassium',
        quantity: 950,
        unit: 'mg',
        dailyPercentage: 26.76
      },
      chloride: {
        name: 'Chloride',
        quantity: 450,
        unit: 'mg',
        dailyPercentage: 19.56
      },
      calcium: {
        name: 'Calcium',
        quantity: 160,
        unit: 'mg',
        dailyPercentage: 22
      },
      phosphorus: {
        name: 'Phosphorus',
        quantity: 121.61,
        unit: 'mg',
        dailyPercentage: 22
      },
      magnesium: {
        name: 'Magnesium',
        quantity: 71,
        unit: 'mg',
        dailyPercentage: 20
      },
      iron: {
        name: 'Iron',
        quantity: 2.109,
        unit: 'mg',
        dailyPercentage: 19
      },
      zinc: {
        name: 'Zinc',
        quantity: 3.29,
        unit: 'mg',
        dailyPercentage: 20
      },
      copper: {
        name: 'Copper',
        quantity: 0.18,
        unit: 'mg',
        dailyPercentage: 12
      },
      manganese: {
        name: 'Manganese',
        quantity: 0.45,
        unit: 'mg',
        dailyPercentage: 15
      },
      selenium: {
        name: 'Selenium',
        quantity: 0.011,
        unit: 'mg',
        dailyPercentage: 15
      },
      molybdenum: {
        name: 'Molybdenum',
        quantity: 0.09,
        unit: 'mg',
        dailyPercentage: 138
      },
      iodine: {
        name: 'Iodine',
        quantity: 40.905,
        unit: 'μg',
        dailyPercentage: 27
      },
      choline: {
        name: 'Choline',
        quantity: 110,
        unit: 'mg',
        dailyPercentage: 27.16
      },
      chromium: {
        name: 'Chromium',
        quantity: 0.07,
        unit: 'mg',
        dailyPercentage: '?'
      }
    }
  },
  {
    id: 4,
    brand: 'Huel',
    product: 'Huel Black Edition',
    site: 'https://huel.com/',
    calories: 400,
    priceAndServings: {
      fullPrice: {
        USD: 83.34,
        GBP: 55.56
      },
      primaryCurrency: 'GBP',
      servings: 34
    },
    accomodatedRestrictions: [
      'vegan', 'lactoseIntolerance', 'nutFree', 'glutenFree', 'kosher', 'halal'
    ],
    macronutrients: {
      fat: {
        total: 17,
        saturated: 2.5,
        trans: 0.5,
        polysaturated: 12
      },
      carbohydrates: {
        total: 24,
        sugars: 5
      },
      protein: 40,
      fibre: 6,
      sodium: 420
    },
    vitaminsAndMinerals: {
      vitaminA: {
        name: 'Vitamin A',
        quantity: 0.178,
        unit: 'mg',
        dailyPercentage: 20
      },
      vitaminD: {
        name: 'Vitamin D',
        quantity: 3.9,
        unit: 'μg',
        dailyPercentage: 20
      },
      vitaminE: {
        name: 'Vitamin E',
        quantity: 3.4,
        unit: 'μg',
        dailyPercentage: 23
      },
      vitaminK: {
        name: 'Vitamin K',
        quantity: 35,
        unit: 'μg',
        dailyPercentage: 29
      },
      vitaminC: {
        name: 'Vitamin C',
        quantity: 59,
        unit: 'mg',
        dailyPercentage: 66
      },
      thiamine: {
        name: 'Thiamine',
        quantity: 0.2,
        unit: 'mg',
        dailyPercentage: 17
      },
      riboflavine: {
        name: 'Riboflavine',
        quantity: 0.3,
        unit: 'mg',
        dailyPercentage: 23
      },
      niacine: {
        name: 'Niacine',
        quantity: 3.2,
        unit: 'mg',
        dailyPercentage: 20
      },
      vitaminB6: {
        name: 'Vitamin B6',
        quantity: 0.3,
        unit: 'mg',
        dailyPercentage: 18
      },
      folicAcid: {
        // Note: it is called Foltate and seems to have inconsistent measurements with the others!
        name: 'Folic Acid',
        quantity: 79,
        unit: 'μg',
        dailyPercentage: 20
      },
      vitaminB12: {
        name: 'Vitamin B12',
        quantity: 0.8,
        unit: 'μg',
        dailyPercentage: 33
      },
      biotine: {
        name: 'Biotine',
        quantity: 0.012,
        unit: 'mg',
        dailyPercentage: 40
      },
      vitaminB5: { // Also called Pantothetic Acid
        name: 'Vitamin B5',
        quantity: 1.2,
        unit: 'mg',
        dailyPercentage: 24
      },
      potassium: {
        name: 'Potassium',
        quantity: 930,
        unit: 'mg',
        dailyPercentage: 20
      },
      chloride: {
        name: 'Chloride',
        quantity: 454,
        unit: 'mg',
        dailyPercentage: 20
      },
      calcium: {
        name: 'Calcium',
        quantity: 260,
        unit: 'mg',
        dailyPercentage: 20
      },
      phosphorus: {
        name: 'Phosphorus',
        quantity: 355,
        unit: 'mg',
        dailyPercentage: 28
      },
      magnesium: {
        name: 'Magnesium',
        quantity: 87,
        unit: 'mg',
        dailyPercentage: 21
      },
      iron: {
        name: 'Iron',
        quantity: 8.9,
        unit: 'mg',
        dailyPercentage: 50
      },
      zinc: {
        name: 'Zinc',
        quantity: 4.6,
        unit: 'mg',
        dailyPercentage: 42
      },
      copper: {
        name: 'Copper',
        quantity: 0.5,
        unit: 'mg',
        dailyPercentage: 56
      },
      manganese: {
        name: 'Manganese',
        quantity: 0.9,
        unit: 'mg',
        dailyPercentage: 39
      },
      selenium: {
        name: 'Selenium',
        quantity: 0.033,
        unit: 'mg',
        dailyPercentage: 60
      },
      molybdenum: {
        name: 'Molybdenum',
        quantity: 0.037,
        unit: 'mg',
        dailyPercentage: 82
      },
      iodine: {
        name: 'Iodine',
        quantity: 30,
        unit: 'μg',
        dailyPercentage: 20
      },
      choline: {
        name: 'Choline',
        quantity: 118,
        unit: 'mg',
        dailyPercentage: 21
      },
      chromium: {
        name: 'Chromium',
        quantity: 0.07,
        unit: 'mg',
        dailyPercentage: '?'
      }
    }
  },
  {
    id: 5,
    brand: 'Huel',
    product: 'Huel Powder',
    site: 'https://huel.com/',
    calories: 400,
    priceAndServings: {
      fullPrice: {
        USD: 72.22,
        GBP: 50
      },
      primaryCurrency: 'GBP',
      servings: 34
    },
    accomodatedRestrictions: [
      'vegan', 'lactoseIntolerance', 'nutFree', 'glutenFree', 'kosher', 'halal'
    ],
    macronutrients: {
      fat: {
        total: 13,
        saturated: 2.5,
        trans: 0.5,
        polysaturated: 8
      },
      carbohydrates: {
        total: 46,
        sugars: 1
      },
      protein: 30,
      fibre: 7,
      sodium: 330
    },
    vitaminsAndMinerals: {
      vitaminA: {
        name: 'Vitamin A',
        quantity: 0.180,
        unit: 'mg',
        dailyPercentage: 20
      },
      vitaminD: {
        name: 'Vitamin D',
        quantity: 4,
        unit: 'μg',
        dailyPercentage: 20
      },
      vitaminE: {
        name: 'Vitamin E',
        quantity: 3.1,
        unit: 'μg',
        dailyPercentage: 21
      },
      vitaminK: {
        name: 'Vitamin K',
        quantity: 36,
        unit: 'μg',
        dailyPercentage: 30
      },
      vitaminC: {
        name: 'Vitamin C',
        quantity: 61,
        unit: 'mg',
        dailyPercentage: 68
      },
      thiamine: {
        name: 'Thiamine',
        quantity: 0.2,
        unit: 'mg',
        dailyPercentage: 17
      },
      riboflavine: {
        name: 'Riboflavine',
        quantity: 0.3,
        unit: 'mg',
        dailyPercentage: 23
      },
      niacine: {
        name: 'Niacine',
        quantity: 3.3,
        unit: 'mg',
        dailyPercentage: 21
      },
      vitaminB6: {
        name: 'Vitamin B6',
        quantity: 0.4,
        unit: 'mg',
        dailyPercentage: 24
      },
      folicAcid: {
        // Note: it is called Foltate and seems to have inconsistent measurements with the others!
        name: 'Folic Acid',
        quantity: 82,
        unit: 'μg',
        dailyPercentage: 21
      },
      vitaminB12: {
        name: 'Vitamin B12',
        quantity: 0.8,
        unit: 'μg',
        dailyPercentage: 33
      },
      biotine: {
        name: 'Biotine',
        quantity: 0.010,
        unit: 'mg',
        dailyPercentage: 33
      },
      vitaminB5: { // Also called Pantothetic Acid
        name: 'Vitamin B5',
        quantity: 1.2,
        unit: 'mg',
        dailyPercentage: 24
      },
      potassium: {
        name: 'Potassium',
        quantity: 960,
        unit: 'mg',
        dailyPercentage: 20
      },
      chloride: {
        name: 'Chloride',
        quantity: 470,
        unit: 'mg',
        dailyPercentage: 20
      },
      calcium: {
        name: 'Calcium',
        quantity: 270,
        unit: 'mg',
        dailyPercentage: 20
      },
      phosphorus: {
        name: 'Phosphorus',
        quantity: 375,
        unit: 'mg',
        dailyPercentage: 30
      },
      magnesium: {
        name: 'Magnesium',
        quantity: 90,
        unit: 'mg',
        dailyPercentage: 21
      },
      iron: {
        name: 'Iron',
        quantity: 8,
        unit: 'mg',
        dailyPercentage: 45
      },
      zinc: {
        name: 'Zinc',
        quantity: 3.7,
        unit: 'mg',
        dailyPercentage: 34
      },
      copper: {
        name: 'Copper',
        quantity: 0.3,
        unit: 'mg',
        dailyPercentage: 34
      },
      manganese: {
        name: 'Manganese',
        quantity: 1.3,
        unit: 'mg',
        dailyPercentage: 57
      },
      selenium: {
        name: 'Selenium',
        quantity: 0.033,
        unit: 'mg',
        dailyPercentage: 60
      },
      molybdenum: {
        name: 'Molybdenum',
        quantity: 0.020,
        unit: 'mg',
        dailyPercentage: 36
      },
      iodine: {
        name: 'Iodine',
        quantity: 30,
        unit: 'μg',
        dailyPercentage: 20
      },
      choline: {
        name: 'Choline',
        quantity: 120,
        unit: 'mg',
        dailyPercentage: 22
      },
      chromium: {
        name: 'Chromium',
        quantity: 0.08,
        unit: 'mg',
        dailyPercentage: 23
      }
    }
  },
  {
    id: 6,
    brand: 'Jimmy Joy',
    product: 'Plenny Shake',
    site: 'https://jimmyjoy.com/',
    calories: 400,
    priceAndServings: {
      fullPrice: {
        EUR: 14
      },
      primaryCurrency: 'EUR',
      servings: 10
    },
    accomodatedRestrictions: [
      'vegan', 'lactoseIntolerance', 'nutFree', 'glutenFree', 'kosher', 'halal'
    ],
    macronutrients: {
      fat: {
        total: 15,
        saturated: 2.3,
        trans: 0, // Doesn't say (?)
        polysaturated: 0 // Doesn't say (?)
      },
      carbohydrates: {
        total: 43,
        sugars: 3.2
      },
      protein: 20,
      fibre: 8.2,
      sodium: 120 // It has salt as 0.30. used a converter to find out...
    },
    vitaminsAndMinerals: {
      vitaminA: {
        name: 'Vitamin A',
        quantity: 0.160,
        unit: 'mg',
        dailyPercentage: 20
      },
      vitaminD: {
        name: 'Vitamin D',
        quantity: 5,
        unit: 'μg',
        dailyPercentage: 100
      },
      vitaminE: {
        name: 'Vitamin E',
        quantity: 4,
        unit: 'μg',
        dailyPercentage: 33
      },
      vitaminK: {
        name: 'Vitamin K',
        quantity: 16,
        unit: 'μg',
        dailyPercentage: 21
      },
      vitaminC: {
        name: 'Vitamin C',
        quantity: 30,
        unit: 'mg',
        dailyPercentage: 38
      },
      thiamine: {
        name: 'Thiamine',
        quantity: 0.4,
        unit: 'mg',
        dailyPercentage: 36
      },
      riboflavine: {
        name: 'Riboflavine',
        quantity: 0.3,
        unit: 'mg',
        dailyPercentage: 23
      },
      niacine: {
        name: 'Niacine',
        quantity: 3.6,
        unit: 'mg',
        dailyPercentage: 23
      },
      vitaminB6: {
        name: 'Vitamin B6',
        quantity: 0.4,
        unit: 'mg',
        dailyPercentage: 24
      },
      folicAcid: {
        name: 'Folic Acid',
        quantity: 60,
        unit: 'μg',
        dailyPercentage: 30
      },
      vitaminB12: {
        name: 'Vitamin B12',
        quantity: 3.2,
        unit: 'μg',
        dailyPercentage: 128
      },
      biotine: {
        name: 'Biotine',
        quantity: 0.010,
        unit: 'mg',
        dailyPercentage: 20
      },
      vitaminB5: { // Also called Pantothetic Acid
        name: 'Vitamin B5',
        quantity: 1.2,
        unit: 'mg',
        dailyPercentage: 24
      },
      potassium: {
        name: 'Potassium',
        quantity: 400,
        unit: 'mg',
        dailyPercentage: 20
      },
      chloride: {
        name: 'Chloride',
        quantity: 279,
        unit: 'mg',
        dailyPercentage: 35
      },
      calcium: {
        name: 'Calcium',
        quantity: 185,
        unit: 'mg',
        dailyPercentage: 23
      },
      phosphorus: {
        name: 'Phosphorus',
        quantity: 140,
        unit: 'mg',
        dailyPercentage: 20
      },
      magnesium: {
        name: 'Magnesium',
        quantity: 75,
        unit: 'mg',
        dailyPercentage: 20
      },
      iron: {
        name: 'Iron',
        quantity: 3.2,
        unit: 'mg',
        dailyPercentage: 23
      },
      zinc: {
        name: 'Zinc',
        quantity: 2,
        unit: 'mg',
        dailyPercentage: 20
      },
      copper: {
        name: 'Copper',
        quantity: 0.4,
        unit: 'mg',
        dailyPercentage: 40
      },
      manganese: {
        name: 'Manganese',
        quantity: 1,
        unit: 'mg',
        dailyPercentage: 50
      },
      selenium: {
        name: 'Selenium',
        quantity: 0.018,
        unit: 'mg',
        dailyPercentage: 33
      },
      molybdenum: {
        name: 'Molybdenum',
        quantity: 0.013,
        unit: 'mg',
        dailyPercentage: 26
      },
      iodine: {
        name: 'Iodine',
        quantity: 30,
        unit: 'μg',
        dailyPercentage: 20
      },
      choline: {
        name: 'Choline',
        quantity: 0,
        unit: 'mg',
        dailyPercentage: 0
      },
      chromium: {
        name: 'Chromium',
        quantity: 0.08,
        unit: 'mg',
        dailyPercentage: 20
      }
    }
  },
  {
    id: 7,
    brand: 'Jimmy Joy',
    product: 'Plenny Shake Active',
    site: 'https://jimmyjoy.com/',
    calories: 400,
    priceAndServings: {
      fullPrice: {
        EUR: 38.50
      },
      primaryCurrency: 'EUR',
      servings: 25
    },
    accomodatedRestrictions: [
      'vegan', 'lactoseIntolerance', 'nutFree', 'glutenFree', 'kosher', 'halal'
    ],
    macronutrients: {
      fat: {
        total: 15,
        saturated: 2.3,
        trans: 0, // Doesn't say (?)
        polysaturated: 0 // Doesn't say (?)
      },
      carbohydrates: {
        total: 35,
        sugars: 3.3
      },
      protein: 26.3,
      fibre: 8.2,
      sodium: 116 // It has salt as 0.29. used a converter to find out...
    },
    vitaminsAndMinerals: {
      vitaminA: {
        name: 'Vitamin A',
        quantity: 0.160,
        unit: 'mg',
        dailyPercentage: 20
      },
      vitaminD: {
        name: 'Vitamin D',
        quantity: 5,
        unit: 'μg',
        dailyPercentage: 100
      },
      vitaminE: {
        name: 'Vitamin E',
        quantity: 4,
        unit: 'μg',
        dailyPercentage: 33
      },
      vitaminK: {
        name: 'Vitamin K',
        quantity: 16,
        unit: 'μg',
        dailyPercentage: 21
      },
      vitaminC: {
        name: 'Vitamin C',
        quantity: 30,
        unit: 'mg',
        dailyPercentage: 38
      },
      thiamine: {
        name: 'Thiamine',
        quantity: 0.4,
        unit: 'mg',
        dailyPercentage: 36
      },
      riboflavine: {
        name: 'Riboflavine',
        quantity: 0.3,
        unit: 'mg',
        dailyPercentage: 23
      },
      niacine: {
        name: 'Niacine',
        quantity: 3.6,
        unit: 'mg',
        dailyPercentage: 23
      },
      vitaminB6: {
        name: 'Vitamin B6',
        quantity: 0.4,
        unit: 'mg',
        dailyPercentage: 24
      },
      folicAcid: {
        name: 'Folic Acid',
        quantity: 60,
        unit: 'μg',
        dailyPercentage: 30
      },
      vitaminB12: {
        name: 'Vitamin B12',
        quantity: 3.2,
        unit: 'μg',
        dailyPercentage: 128
      },
      biotine: {
        name: 'Biotine',
        quantity: 0.010,
        unit: 'mg',
        dailyPercentage: 20
      },
      vitaminB5: { // Also called Pantothetic Acid
        name: 'Vitamin B5',
        quantity: 1.2,
        unit: 'mg',
        dailyPercentage: 24
      },
      potassium: {
        name: 'Potassium',
        quantity: 400,
        unit: 'mg',
        dailyPercentage: 20
      },
      chloride: {
        name: 'Chloride',
        quantity: 279,
        unit: 'mg',
        dailyPercentage: 35
      },
      calcium: {
        name: 'Calcium',
        quantity: 185,
        unit: 'mg',
        dailyPercentage: 23
      },
      phosphorus: {
        name: 'Phosphorus',
        quantity: 140,
        unit: 'mg',
        dailyPercentage: 20
      },
      magnesium: {
        name: 'Magnesium',
        quantity: 75,
        unit: 'mg',
        dailyPercentage: 20
      },
      iron: {
        name: 'Iron',
        quantity: 3.2,
        unit: 'mg',
        dailyPercentage: 23
      },
      zinc: {
        name: 'Zinc',
        quantity: 2,
        unit: 'mg',
        dailyPercentage: 20
      },
      copper: {
        name: 'Copper',
        quantity: 0.4,
        unit: 'mg',
        dailyPercentage: 40
      },
      manganese: {
        name: 'Manganese',
        quantity: 1,
        unit: 'mg',
        dailyPercentage: 50
      },
      selenium: {
        name: 'Selenium',
        quantity: 0.018,
        unit: 'mg',
        dailyPercentage: 33
      },
      molybdenum: {
        name: 'Molybdenum',
        quantity: 0.013,
        unit: 'mg',
        dailyPercentage: 26
      },
      iodine: {
        name: 'Iodine',
        quantity: 30,
        unit: 'μg',
        dailyPercentage: 20
      },
      choline: {
        name: 'Choline',
        quantity: 0,
        unit: 'mg',
        dailyPercentage: 0
      },
      chromium: {
        name: 'Chromium',
        quantity: 0.08,
        unit: 'mg',
        dailyPercentage: 20
      }
    }
  },
  {
    id: 8,
    brand: 'Super Body Fuel',
    product: 'Milk Fuel (any flavour)',
    site: 'https://superbodyfuel.com/',
    calories: 500,
    priceAndServings: {
      fullPrice: {
        USD: 50
      },
      primaryCurrency: 'USD',
      servings: 40
    },
    accomodatedRestrictions: [
      'vegan', 'lactoseIntolerance', 'nutFree', 'glutenFree', 'kosher', 'halal'
    ],
    macronutrients: {
      fat: {
        total: 20,
        saturated: 10,
        trans: 0,
        polysaturated: 0 // Doesn't say (?)
      },
      carbohydrates: {
        total: 59,
        sugars: 23 // (g) confirm if measurements match
      },
      protein: 24,
      fibre: 8,
      sodium: 575 // (mg)
    },
    vitaminsAndMinerals: {
      vitaminA: {
        name: 'Vitamin A',
        quantity: 1.100,
        unit: 'mg',
        dailyPercentage: 120
      }, // here
      vitaminD: {
        name: 'Vitamin D',
        quantity: 20,
        unit: 'μg',
        dailyPercentage: 100
      },
      vitaminE: {
        name: 'Vitamin E',
        quantity: 4.2,
        unit: 'mg',
        dailyPercentage: 25
      },
      vitaminK: {
        name: 'Vitamin K',
        quantity: 60,
        unit: 'μg',
        dailyPercentage: 50
      },
      vitaminC: {
        name: 'Vitamin C',
        quantity: 90,
        unit: 'mg',
        dailyPercentage: 100
      },
      thiamine: {
        name: 'Thiamine',
        quantity: 1.1,
        unit: 'mg',
        dailyPercentage: 90
      },
      riboflavine: {
        name: 'Riboflavine',
        quantity: 1.5,
        unit: 'mg',
        dailyPercentage: 110
      },
      niacine: {
        name: 'Niacine',
        quantity: 5.1,
        unit: 'mg',
        dailyPercentage: 30
      },
      vitaminB6: {
        name: 'Vitamin B6',
        quantity: 1,
        unit: 'mg',
        dailyPercentage: 60
      },
      folicAcid: { // Folate??
        name: 'Folic Acid',
        quantity: 140,
        unit: 'μg',
        dailyPercentage: 35
      },
      vitaminB12: {
        name: 'Vitamin B12',
        quantity: 4.6,
        unit: 'μg',
        dailyPercentage: 190
      },
      biotine: {
        name: 'Biotine',
        quantity: 30,
        unit: 'μg',
        dailyPercentage: 100
      },
      vitaminB5: { // Also called Pantothetic Acid
        name: 'Vitamin B5',
        quantity: 6.9,
        unit: 'mg',
        dailyPercentage: 130
      },
      potassium: {
        name: 'Potassium',
        quantity: 1175,
        unit: 'mg',
        dailyPercentage: 25
      },
      chloride: {
        name: 'Chloride',
        quantity: 0,
        unit: 'mg',
        dailyPercentage: 0
      },
      calcium: {
        name: 'Calcium',
        quantity: 575,
        unit: 'mg',
        dailyPercentage: 40
      },
      phosphorus: {
        name: 'Phosphorus',
        quantity: 580,
        unit: 'mg',
        dailyPercentage: 45
      },
      magnesium: {
        name: 'Magnesium',
        quantity: 210,
        unit: 'mg',
        dailyPercentage: 50
      },
      iron: {
        name: 'Iron',
        quantity: 5.5,
        unit: 'mg',
        dailyPercentage: 30
      },
      zinc: {
        name: 'Zinc',
        quantity: 5.8,
        unit: 'mg',
        dailyPercentage: 50
      },
      copper: {
        name: 'Copper',
        quantity: 0.7,
        unit: 'mg',
        dailyPercentage: 80
      },
      manganese: {
        name: 'Manganese',
        quantity: 2,
        unit: 'mg',
        dailyPercentage: 80
      },
      selenium: {
        name: 'Selenium',
        quantity: 70,
        unit: 'μg',
        dailyPercentage: 130
      },
      molybdenum: {
        name: 'Molybdenum',
        quantity: 65,
        unit: 'μg',
        dailyPercentage: 150
      },
      iodine: {
        name: 'Iodine',
        quantity: 45,
        unit: 'μg',
        dailyPercentage: 30
      },
      choline: {
        name: 'Choline',
        quantity: 220,
        unit: 'mg',
        dailyPercentage: 40
      },
      chromium: {
        name: 'Chromium',
        quantity: 35,
        unit: 'μg',
        dailyPercentage: 100
      }
    }
  },
]
